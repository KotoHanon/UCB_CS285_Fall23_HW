...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        mean = self.mean_net(observation) # mean.shape: [action_dim]
        std = torch.exp(self.logstd) # std.shape: [action_dim]
        dist = torch.distributions.Normal(mean, std) # Gaussian
        action = dist.rsample() # sample from Gaussian with re-parameteration
        return action
